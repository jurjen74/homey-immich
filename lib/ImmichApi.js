...
class ImmichApi {
  ...
  constructor({ url, apiKey }) {
    ...
    this._lib = this._baseUrl.startsWith('https://') ? https : http;
  }
      ...
      const req = this._lib.request(endpoint, { method, headers, timeout: 10000 }, (res) => {
        ...
        res.on('end', () => {
          if (res.statusCode >= 200 && res.statusCode < 300) {
            ...
            try {
              const p = JSON.parse(data); msg = p.message ?? p.error ?? msg;
            } catch { /* */ }
  ...
  searchAssets({
    createdAfter, type, personIds, albumIds, page = 1, size = 100, order = 'asc', withPeople = true,
  } = {}) {
    const body = {
      page, size, order, withPeople,
    };
  ...
  // Memories — accepts a 'YYYY-MM-DD' date or a Date. Immich v3 requires a full
  // ISO date-time for the `for` parameter; a date-only string fails validation
  // (400 "Validation failed"), so normalize to midnight UTC of that date.
  getMemories(date) {
    let forParam = date;
    if (date instanceof Date) forParam = date.toISOString();
    else if (typeof date === 'string' && /^\d{4}-\d{2}-\d{2}$/.test(date)) forParam = `${date}T00:00:00.000Z`;
    return this._request('GET', '/memories', { query: { for: forParam } });
  }
  ...
  createSharedLink(assetIds) {
    return this._request('POST', '/shared-links', {
      body: {
        type: 'INDIVIDUAL', assetIds, allowDownload: true, showMetadata: true,
      },
    });
  }
  ...
  // Uses POST /search/random (returns a plain array of assets). The old
  // GET /assets/random was removed in Immich v3; /search/random is available
  // across all supported versions (2.7.5+).
  getRandomAssets(count = 1) {
    return this._request('POST', '/search/random', { body: { size: count } });
  }

  // The album response no longer embeds its `assets` array as of Immich v3, so
  // fetch an album's assets via metadata search (works on 2.7.5+). Paginated up
  // to 50 × 100 = 5000 assets.
  async getAlbumAssets(albumId) {
    const all = [];
    for (let page = 1; page <= 50; page++) {
      const res = await this.searchAssets({
        albumIds: [albumId], page, size: 100, withPeople: false,
      });
  ...
  getThumbnail(id, size = 'thumbnail') {
    ...
    return new Promise((resolve, reject) => {
      ...
      req.on('error', reject);
      req.on('timeout', () => {
        req.destroy(); reject(new Error('Thumbnail timed out'));
      });
      req.end();
    });
  }

  getThumbnailStream(id, size = 'preview') {
    const url = `${this._baseUrl}/api/assets/${encodeURIComponent(id)}/thumbnail?size=${encodeURIComponent(size)}`;
    const headers = { 'x-api-key': this._apiKey };
    return new Promise((resolve, reject) => {
      const req = this._lib.request(url, { method: 'GET', headers, timeout: 15000 }, (res) => {
  ...
  createAlbumSharedLink(albumId) {
    return this._request('POST', '/shared-links', {
      body: {
        type: 'ALBUM', albumId, allowDownload: true, showMetadata: true,
      },
    });
  }

  updateAsset(id, updates) {
    return this._request('PUT', `/assets/${id}`, { body: updates });
  }
  ...
  getTrashCount() {
    return this._request('GET', '/assets/statistics', { query: { isTrashed: true } })
      .then((r) => r?.total ?? 0);
  }
